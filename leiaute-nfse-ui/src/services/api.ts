...
class APIClient {
  ...
  async getServices(limit: number = 10, offset: number = 0): Promise<PaginatedResponse<Servico>> {
  ...
  async getServiceRules(codigo: number): Promise<{ codigo_servico: number, total_regras: number, regras: Regra[] }> {
  ...
  async getRulesByLevel(nivel: number, limit: number = 20): Promise<{ nivel: number, total: number, items: Regra[] }> {
  ...
  async getRulesByErrorCode(codigo_erro: string): Promise<{ codigo_erro: string, total: number, items: Regra[] }> {
    const response = await this.client.get(`/rules/filtro/por-erro/${codigo_erro}`)
    return response.data
  }
  ...
  async compareScenarios(ids: number[]): Promise<{ cenarios_comparados: number, cenarios: Cenario[], diferencas: any }> {
    const params = new URLSearchParams()
    ids.forEach(id => params.append('cenarios_ids', id.toString()))
    
    const response = await this.client.post('/scenarios/compare', null, { params })
    return response.data
  }

  async getDomesticScenarios(limit: number = 10): Promise<{ cenarios_encontrados: number, items: Cenario[] }> {
  ...
  async getExportScenarios(limit: number = 10): Promise<{ cenarios_encontrados: number, items: Cenario[] }> {
  ...
  async search(
    ...
    tipo?: string,
    limit: number = 20
  ): Promise<{ query: string, filtro_tipo: string | null, total_resultados: number, items: SearchResult[] }> {
  ...
  async searchByServiceCode(codigo: number): Promise<{ encontrado: boolean, servico?: any }> {
    const response = await this.client.get(`/search/codigo/${codigo}`)
    return response.data
  }

  async searchByErrorCode(codigo_erro: string): Promise<{ encontrado: boolean, codigo_erro: string, total: number, regras: any[] }> {
